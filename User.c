...
Coordonee** getType(Coordonee* centralPos,Plateau* plat,Plateau* ennemyPlat){
    ...
    if(!plat->canSpecialShoot || ( (plat->boatAlive[1]->used || !isAliveBoat(plat->boatAlive[1],plat)) && (plat->boatAlive[3]->used || !isAliveBoat(plat->boatAlive[3],plat)) && (plat->boatAlive[4]->used || !isAliveBoat(plat->boatAlive[4],plat)) ) )
